...
def check_mesh(vertices:list[tuple[float, float, float]], faces: list[tuple[int, int, int]], quick=False) -> bool:
    ...
    if not mesh.is_watertight:
        ...
        if len(non_manifold_edges) > 0:
            ...
            view_mesh(mesh, title= 'Edges in red are non-manifold edges. (adjacent count > 2)',
                check_edges=non_manifold_edges, show_normals = False)
            return False
            ...
            view_mesh(mesh, title= 'Edges in red are boundary edges. (adjacent count = 1)',
                check_edges=boundary_edges, show_normals = False)
            return False
    ...
    if len(non_manifold_vertices) > 0:
        ...
        print('Your object has non-manifold vertices, look at these indices in your vertices list:')
        print(non_manifold_vertices)
        print('Usual causes: an extraneous point, a degenerate triangle (zero area), or a single point of contact.')
        view_mesh(mesh, title= 'Vertices in red are non-manifold vertices.',
            check_vertices=non_manifold_vertices, show_normals = False)
        return False
    ...
    if not mesh.is_winding_consistent:
        ...
        print("View each mesh listed and look for red arrows that point inside your mesh.")
        print("The easiest way to correct the bad winding is reverse the last two numbers in the face.")
        print(f"The faces that need to be manually checked are: {bad_winding}")
        view_mesh(mesh, title= 'Faces with arrow pointing inside have an incorrect winding',
            check_faces=bad_winding, show_normals = True)
        return False

    if mesh.volume < 0:
        if quick:
            return "probably all windings are bad"
        print('Your 3d object is watertight (is manifold) and your winding is consistent.')
        print("Your objects volume is less than zero.")
        print('This almost certainly means that ALL you faces have the wrong winding.')
        view_mesh(mesh, title= 'Faces with arrow pointing inside have an incorrect winding',
            check_faces=list(range(0, len(mesh.faces))), show_normals = True)
        return False
...
def quick_check_mesh(vertices:list[tuple[float, float, float]], faces: list[tuple[int, int, int]]) -> bool:
    return check_mesh(vertices, faces, True)
    
def view_mesh(mesh, title, check_faces=[], check_edges=[], check_vertices=[], show_normals = False):
    ...
    if len(check_vertices) > 0:
        for vi in check_vertices:
            x, y, z = v[vi]
            ax.scatter(x, y, z, color='red', s=100, zorder=5, label='Highlighted Vertex')
    ...
    if show_normals:
        ...
        for i, face in enumerate(f):
            ...
            if norm_len > 0:
                ...
                ax.quiver(*centroid, *norm_vec, color='red', length=0.75, arrow_length_ratio=0.4)
