...
def extrude_batch(
    ...
    if cur[0] == 0:
        cur = paths[1]
        cur_idx = 1
        add_zero_cap(cur_z, cur, top=False)
    else:
        add_cap(cur_z, cur, top=False)
    ...
    if capped_wall != None:
        ...
        add_zero_cap(cur_z, paths[-2], top=True)
    else:
        add_cap(cur_z, paths[-1], top=True)
